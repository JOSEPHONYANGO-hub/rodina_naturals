    ...
    <aside className="h-fit overflow-hidden rounded-[12px] border border-[#d7e0ea] bg-white shadow-[0_10px_30px_rgba(34,34,34,0.04)]">
      ...
      <div className="p-4">
        ...
        <form className="relative" onSubmit={submitSearch}>
          ...
          <input
            className="field h-10 rounded-lg pl-10 pr-12 text-sm"
            placeholder="Search products"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
          />
          <button type="submit" className="absolute right-1 top-1 grid h-8 w-8 place-items-center rounded-md bg-[#a81723] text-white transition hover:bg-[#7d111b]" aria-label="Apply product search">
